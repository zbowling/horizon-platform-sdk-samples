...
class ApplicationLifecycleViewModelTest {

  @Test
  fun `initial ui state has correct defaults`() {
    val state = ApplicationLifecycleUiState()
  ...
  fun `ui state copy preserves result message`() {
    val state = ApplicationLifecycleUiState(resultMessage = "test result")
  ...
  fun `ui state copy preserves error message`() {
    val state = ApplicationLifecycleUiState(errorMessage = "test error")
    val updated = state.copy(isLoading = false)
