...
class RateAndReviewViewModelTest {

  @Test
  fun `initial ui state has correct defaults`() {
    val state = RateAndReviewUiState()
  ...
  fun `ui state copy preserves result message`() {
    val state = RateAndReviewUiState(resultMessage = "test result")
  ...
  fun `ui state copy preserves error message`() {
    val state = RateAndReviewUiState(errorMessage = "test error")
    val updated = state.copy(isLoading = false)
