...
class DeviceIntegrityViewModelTest {

  @Test
  fun `initial ui state has correct defaults`() {
    val state = DeviceIntegrityUiState()
  ...
  fun `ui state copy preserves result message`() {
    val state = DeviceIntegrityUiState(resultMessage = "test result")
  ...
  fun `ui state copy preserves error message`() {
    val state = DeviceIntegrityUiState(errorMessage = "test error")
    val updated = state.copy(isLoading = false)
